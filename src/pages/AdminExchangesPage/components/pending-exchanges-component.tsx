...
const PendingExchangesComponent: React.FC = () => {
    ...
    const fetchExchanges = async () => {
        try {
            setLoading(true);
            const response = await api.getPendingExchanges();
            const exchanges = response.data || [];
            
            const filtered = exchanges.filter((exchange: ExchangeResponse) =>
                exchange.status.name === 'TROCA SOLICITADA' || exchange.status.name === 'DEVOLUÇÃO SOLICITADA'
            );
            setPendingExchanges(filtered);
        } catch (err) {
            setError("Erro ao carregar trocas pendentes");
        ...
        return (
            <Box sx={{
                ...
                <Typography variant="h6" color="text.secondary">
                    Nenhuma troca/devolução pendente encontrada
                </Typography>
            </Box>
        );
    }
...
export default PendingExchangesComponent;
