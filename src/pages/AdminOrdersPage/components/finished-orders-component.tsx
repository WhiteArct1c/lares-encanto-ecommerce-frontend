...
const FinishedOrdersComponent: React.FC = () => {
    const [finishedOrders, setFinishedOrders] = useState<OrderCreateResponse[]>([]);
    ...
    const fetchOrders = async () => {
        try {
            ...
            let response;
            try {
                response = await api.getAllFinishedOrders();
                ...
                if (err?.response?.status === 404 || err?.response?.status === 400) {
                    ...
                    const filtered = allOrders.filter((order: OrderCreateResponse) =>
                        FINISHED_STATUSES.includes(order.status.name.toUpperCase())
                    );
                    setFinishedOrders(filtered);
                } else {
                    throw err;
                }
            }
        } catch (err) {
            setError("Erro ao carregar pedidos concluídos");
...
export default FinishedOrdersComponent;
